...
Description: Refactored to include all helper utilities and the improved heuristic in a single self‑contained block.
Generated:   2026-03-11T15:00:00.778752
"""
...
Key enhancements:
  * Positive incentive for movement proportional to distance reduction.
  * Higher base score for movement to outweigh low‑value informational actions.
  * Penalise repeated “take” actions when inventory does not change.
  * Small penalty for look/inventory actions that do not change the observation.
  * Stronger door‑toggle pruning (removes the opposite close/open action from both
    parent and child untried‑action containers).
  * Reduced random tie‑breaker magnitude.
"""
...
def _score_action(action: str, state) -> float:
    ...
    # Base scores per category (tuned)
    base_scores = {
        "move": 1.5,   # give movement a positive baseline
        "open": 0.0,
        "close": -2.0,
        "take": 1.5,
        "read": 1.0,
    ...
    # ------------------------------------------------------------------
    # Movement: reward reduction in distance to goal (if distance info exists)
    # ------------------------------------------------------------------
    if cat == "move":
        try:
            ...
            if before_dist is not None and after_dist is not None:
                dist_gain = max(0, before_dist - after_dist)
                score += dist_gain
                if after_dist == 0:
                    score += 10.0
        except Exception:
    ...
    # ------------------------------------------------------------------
    # Opening doors: reward if it brings us closer to the goal
    # ------------------------------------------------------------------
    if cat == "open":
        try:
            before = state.distance_to_goal()
            after_dist = after.distance_to_goal()
            if before is not None and after_dist is not None and after_dist < before:
                score += 5.0 + (before - after_dist)
        except Exception:
            pass

    # ------------------------------------------------------------------
    # Reading the map: incentivise only when map not yet read
    # ------------------------------------------------------------------
    if cat == "read":
    ...
    # ------------------------------------------------------------------
    # No‑op actions: small penalty if they do not change visible observation
    # ------------------------------------------------------------------
    if cat == "noop":
        try:
    ...
    # ------------------------------------------------------------------
    # Small random tie‑breaker (reduced magnitude)
    # ------------------------------------------------------------------
