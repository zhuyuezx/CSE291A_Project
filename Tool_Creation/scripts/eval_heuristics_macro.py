"""
Fast heuristic evaluation — Sokoban Macro-Push variant.

Identical to eval_heuristics.py but reads from sokoban_macro_hyperparams.py
instead of default_hyperparams.py.

Usage:
    python scripts/eval_heuristics_macro.py
    python scripts/eval_heuristics_macro.py --levels level1,level2,level3 --runs 10
    python scripts/eval_heuristics_macro.py --baseline-only
    python scripts/eval_heuristics_macro.py --iters 200 --runs 3
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate Sokoban macro-push heuristics (no LLM)."
    )
    p.add_argument(
...
def _load_config():
    ...
    hp_path = ROOT / "MCTS_tools" / "hyperparams" / HYPERPARAMS_FILE
    ...
    tl_name = getattr(hp_mod, "TRAINING_LOGIC", "sokoban_macro_training")
...
def main() -> None:
    ...
    game_class  = getattr(game_module, getattr(hp_mod, "GAME_CLASS", "SokobanMacro"))
    ...
    # ── Print config ─────────────────────────────────────────────────
    print("=" * 70)
    print("  SOKOBAN MACRO-PUSH HEURISTIC EVAL")
    print("=" * 70)
