...
class PromptBuilder:
    ...
    def _build_task_section(self) -> str:
        ...
        return (
            ...
            f"  • The code block must be SELF-CONTAINED: include every\n"
            f"    import it uses and define every helper it calls.\n"
            f"  • Do NOT assume helpers exist in sibling generated files.\n"
            f"    If you call _heuristic/_score/etc., define it in the same\n"
            f"    code block or explicitly import it.\n"
            f"  • Do NOT assume a mutable container is a set unless the\n"
            f"    provided code proves it. If a value may be a list, do not\n"
            f"    call set-only methods like discard/add.\n"
            f"  • Only use methods and attributes that are explicitly visible\n"
            f"    in the provided tool code, state API, or traces.\n"
            f"  • Before finalizing, mentally check for NameError risks:\n"
            f"    missing imports, missing helper functions, missing constants.\n"
            f"  • Before finalizing, mentally check for AttributeError risks:\n"
            f"    wrong container method, wrong node/state field, wrong return type.\n"
            ...
            f"```\n\n"
            f"The first non-empty line of your response MUST be `ACTION: ...`.\n"
            f"Do not add commentary before the header.\n\n"
            ...
            f"- The code must be self-contained: all imports, helper functions,\n"
            f"  and constants referenced by the main function must appear in\n"
            f"  the same code block."
        )
    ...
    def _build_critique_task_section(self) -> str:
        ...
        return (
            ...
            f"  - Ensure the FINAL code is self-contained: include every\n"
            f"    import, helper function, and constant it references.\n"
            f"  - Eliminate NameError risks caused by missing imports or\n"
            f"    helper definitions.\n"
            f"  - Eliminate AttributeError risks caused by set/list confusion\n"
            f"    or by assuming unavailable node/state methods.\n"
            ...
            "```\n\n"
            f"The first non-empty line after the optional CRITIQUE block must be\n"
            f"`ACTION: ...`. Do not add extra commentary before the header.\n\n"
            f"Rules for the header fields:\n"
