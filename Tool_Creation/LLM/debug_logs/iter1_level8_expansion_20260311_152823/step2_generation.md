...
DESCRIPTION: Added delta‑distance penalty, target‑placement bonus, stronger wall‑line deadlock detection, and increased walk‑cost weight to prioritize pushes that truly advance the solution while pruning more hopeless actions.
```python
def default_expansion(node):
    """
