...
DESCRIPTION: Added forward walk‑cost estimate, fixed UCB sign, dynamic target bonus, richer dead‑lock detection, and a novelty term to improve exploration and prioritize promising pushes.
```python
def default_selection(root, exploration_weight: float = 1.41):
    """
