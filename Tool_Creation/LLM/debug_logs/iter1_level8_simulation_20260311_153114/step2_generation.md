...
DESCRIPTION: Added stronger walk‑cost weight, larger target‑completion bonus, dead‑lock detection (corner & wall‑line), and non‑linear scaling to widen reward spread and penalise hopeless states.
```python
def default_simulation(state, perspective_player: int, max_depth: int = 0) -> float:
    """
