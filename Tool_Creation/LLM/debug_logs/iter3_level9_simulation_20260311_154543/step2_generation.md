...
DESCRIPTION: Enriched dead‑lock detection (2×2 blocks, frozen boxes, dead‑square cells), added depth‑discount, broadened reward scaling and walk‑cost weight, and penalised boxes on dead squares to give the rollout a more discriminative signal.
```python
def default_simulation(state, perspective_player: int, max_depth: int = 0) -> float:
    """
