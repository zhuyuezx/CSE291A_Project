...
CRITIQUE:
- Exploration term uses `log(parent_visits)` where `parent_visits` can be 1 at the start, yielding `log(1)=0` and thus zero exploration for all children (undoes the intended UCB effect).
- Minor redundancy: `import math` is already done at module level; re‑importing inside the function is unnecessary but harmless.

ACTION: modify
FILE_NAME: selection.py
FUNCTION_NAME: default_selection
DESCRIPTION: Fix exploration term to ensure a positive log value and clean up minor redundancy.
```python
def default_selection(node, exploration_weight: float = 1.41):
    """
    A*-guided tree walk with lightweight UCB exploration and Sokoban‑specific
