...
DESCRIPTION: Adjusted depth discounting to leaf‑relative, raised γ, reduced step‑penalty, and applied solved‑bonus after discount to keep strong reward signals for deep solutions.
```python
def default_backpropagation(node, reward: float) -> None:
    """
