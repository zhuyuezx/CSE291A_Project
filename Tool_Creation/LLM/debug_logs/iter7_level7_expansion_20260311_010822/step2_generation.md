...
DESCRIPTION: added true walk‑cost based g, richer heuristic (walk + box distance), lightweight static dead‑lock detection, and only permanently prune dead‑locked actions while keeping visited‑pruned actions for later consideration.
```python
"""
