...
DESCRIPTION: Align visited‑table handling with expansion, replace inconsistent g‑score updates, use step‑based exponential discount, and maintain node.value as an average to avoid value/visits bias.
```python
def default_backpropagation(node, reward: float) -> None:
    """
