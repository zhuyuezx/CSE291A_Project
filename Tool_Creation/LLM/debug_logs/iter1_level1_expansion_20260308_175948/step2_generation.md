...
DESCRIPTION: Added stricter push ordering, extra cheap dead‑lock patterns (2×2 block, corridor block, simple frozen check), player‑reachability pruning, richer transposition key (includes step count), and bounded per‑root cache size to prevent memory blow‑up.
```python
def default_expansion(node):
    """
    Expand one *promising* untried action from the given node.
