...
Traceback (most recent call last):
  ...
  File "/Users/hrzhang/Desktop/WI26/CSE291A/Project/code/CSE291A_Project/Tool_Creation/MCTS_tools/expansion/expansion.py", line 32, in default_expansion
    ...
    ^^^^^^^^^^^^^^^^^^^
AttributeError: 'MCTSNode' object has no attribute '_mcts_root_key' and no __dict__ for setting new attributes
...
**FILE_NAME:** expansion.py  
**FUNCTION_NAME:** default_expansion  
**DESCRIPTION:** removed illegal assignment to a slot‑based `MCTSNode` and relied on the agent’s internal bookkeeping for the root key.
...
def default_expansion(node):
    ...
    # The unique identifier for the whole Sokoban instance
    root_key = root.state.state_key()

    # The agent (`_ag`) is responsible for storing the current root key.
    # If the key changed we reset its internal tables.
    if getattr(_ag, "_root_key", None) != root_key:
        _ag.reset(root_key)               # updates _ag._root_key internally
