...
Traceback (most recent call last):
  ...
  File "/Users/hrzhang/Desktop/WI26/CSE291A/Project/code/CSE291A_Project/Tool_Creation/MCTS_tools/simulation/simulation.py", line 16, in <module>
    if _KEY not in sys.modules:
                   ^^^
...
**FILE_NAME:** simulation.py  
**FUNCTION_NAME:** default_simulation  
**DESCRIPTION:** Fixed missing `sys` import (which caused a `NameError`) and added a brief doc‑string; otherwise the heuristic logic is unchanged and conforms to the public `GameState` API.  
...
def default_simulation(state, perspective_player: int, max_depth: int = 0) -> float:
    ...
    The function combines several heuristics (box‑to‑target distance,
    unreachable boxes, walk distance to a push position, and corner dead‑lock
    detection) into a smooth reward ``exp(-λ·h)``.  It now safely imports
    ``sys`` (required by the module‑level code) and follows the public
    GameState API.
    """
    # --------------------------------------------------------------------- #
    # The module that defines ``_KEY`` may reference ``sys`` at import time.
    # Import it here to guarantee the name exists when the function is
    # executed (prevents the NameError observed in the runtime error).
    # --------------------------------------------------------------------- #
    import sys  # noqa: F401

    # ---- 0️⃣ Terminal handling -------------------------------------------
    if state.is_terminal():
        # ``returns`` gives a tuple/list of rewards for each player.
        ret = state.returns()[perspective_player]   # 1.0 for solved, 0.0 otherwise
        # Penalise only clear dead‑locks; keep step‑limit terminations at 0.0
        if ret == 0.0 and _simple_corner_deadlock(state):
            return -0.1
    ...
    # ---- 1️⃣ Box‑to‑target Manhattan distance -------------------------------
    # ``total_box_distance`` already returns Σ min‑Manhattan per box.
    box_dist = state.total_box_distance()

    # ---- 2️⃣ Unreachable‑box count -----------------------------------------
    walls = state.walls          # frozenset of wall coordinates
    boxes = state.boxes          # set of box coordinates
    reachable = _bfs_reachable(state.player, walls, boxes)
    unreachable_cnt = sum(1 for b in boxes if b not in reachable)

    # ---- 3️⃣ Minimal walk distance to a legal push position ----------------
    # ``legal_actions`` returns an iterable of actions; each action is
    # assumed to be a tuple ``(push_position, ...)`` where the first element
    # is the square the player must stand on to execute the push.
    min_walk = None
    for action in state.legal_actions():
        push_pos = action[0]                       # cell where player must stand
        if push_pos in reachable:
            ...
            if min_walk is None or d < min_walk:
                min_walk = d
    # If no push is currently possible the state should be terminal,
