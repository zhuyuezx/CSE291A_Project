...
**Description:** Removed the illegal `from __future__ import annotations` line (which must be the first line of a file) and updated the function to use the public `player` attribute directly. The heuristic and overall logic remain unchanged.
...
def default_expansion(node):
    ...
    # 1. Gather parent information
    # ------------------------------------------------------------------ #
    # `steps` is the cumulative number of player moves taken to reach this state
    ...
    pruned = []          # permanently discarded actions
