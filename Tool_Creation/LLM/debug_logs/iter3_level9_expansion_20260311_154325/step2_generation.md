...
DESCRIPTION: Added richer dead‑lock detection (dead squares, frozen boxes, 2×2 blocks), refined distance‑increase to focus on the moved box, discounted farthest‑box penalty, dynamic distance weighting, and a stronger diversification term (random jitter) to prevent early lock‑in on a single push.
```python
def default_expansion(node):
    """
    Expanded heuristic for macro‑push actions with additional
