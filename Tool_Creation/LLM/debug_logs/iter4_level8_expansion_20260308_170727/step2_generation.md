...
DESCRIPTION: Added push bias, player‑to‑nearest‑box distance improvement, wall‑line dead‑lock pruning, and a transposition key that includes the player position to prevent discarding useful approach moves.
```python
def default_expansion(node):
    """
    Expand one *promising* untried action from the given node.
